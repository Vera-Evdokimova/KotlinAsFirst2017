...
fun diameter(vararg points: Point): Segment {
    ...
    var max = 0.0
    var g = Segment(points[0], points[1])
    for (p1 in points) {
        for (p2 in points.filter { it != p1 }) {
            if (max < p1.distance(p2)) {
                max = p1.distance(p2)
                g = Segment(p1, p2)
            }
        }
...
fun bisectorByPoints(a: Point, b: Point): Line {
    var m = if (lineByPoints(a, b).angle <= Math.PI / 2) Math.PI / 2 + lineByPoints(a, b).angle
    else lineByPoints(a, b).angle - Math.PI / 2
    if (m == Math.PI) m = 0.0
    val l = Point((a.x + b.x) / 2, (a.y + b.y) / 2)
    return Line(l, m)
}
